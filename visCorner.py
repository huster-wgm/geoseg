...
def main(args):
    ...
    if not os.path.exists(os.path.join(Result_DIR, 'corner')):
        os.makedirs(os.path.join(Result_DIR, 'corner'))
    ...
    for checkpoint in args.checkpoints:
        ...
        final_img = result_imgs[0::2] + result_imgs[1::2]
        final_img = vision.slices_to_img(final_img, [2, len(corners)])
        final_img = vision.add_color_bar(final_img, args.spaces[1], 'white')
        name = "{}_{}_corners.png".format(
            checkpoint.split('_')[0], args.target)
        imsave(os.path.join(Result_DIR, 'corner', name), final_img)
        print("Saving {} ...".format(name))

        perform = ["%0.3f" % (value / len(corners))
                   for value in [oa, precision, recall, f1, jac, kappa]]

        perform = pd.DataFrame([[time.strftime("%h%d_%H"), checkpoint, args.target] + perform],
                               columns=["time", "checkpoint", "segmap"] + metrics_name)

        # save performance
        log_path = os.path.join(Result_DIR, "corner-performs.csv")
        if os.path.exists(log_path):
            performs = pd.read_csv(log_path)
        else:
            performs = performs = pd.DataFrame([])
