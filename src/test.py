...
def main(args):
    ...
    for checkpoint in args.checkpoints:
        ...
        for idx in range(len(data)):
            ...
            if args.cuda:
                y_pred = y_pred.cpu()
            assert y_pred.shape[0] == x.shape[0], "All data should be iterated."
            del x
            ...
            imsave(os.path.join(Save_DIR, data.files[idx]), pred_img, compress=6)
            # N tensor 2 one
            ...
            for idx, evaluator in enumerate(evaluators):
                ...
                performs[idx] += evaluator(y_pred, y)[0].item()

        performs = ["%0.3f" % float(value / len(data)) for value in performs]
        performs = pd.DataFrame([[time.strftime("%h_%d_%H"), checkpoint.split('.pth')[0]] + performs],
