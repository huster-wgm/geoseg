...
def set_trainer(args, method):
    ...
    if "MCFCN" in args.net:
        return mcTrainer(args, method, is_multi=True, loss=args.loss)
    elif "BRNet" in args.net:
        args.alpha = 0.5
        return brTrainer(args, method, is_multi=True, loss=args.loss)
    else:
        return Trainer(args, method, is_multi=False, loss=args.loss)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-loss', type=str, default='CELoss',
                        help='loss usded for training model ')
